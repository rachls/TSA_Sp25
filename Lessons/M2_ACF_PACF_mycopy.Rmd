...
ggplot(data = inflow_data, aes(x=my_date))+
  geom_line(aes(y = HP1), colour = "red")+
  geom_line(aes(y = HP2), colour = "blue")+
  geom_line(aes(y = HP3))+
  labs(y = "HP",
       x = "Date")
...
```{r}
#exclude date column because it tries to convert it into a times series
ts_inflow_data <- ts(data = inflow_data[,2:4], 
                     start = c(1931,1), #when does the time start
                     frequency = 12) #how often do the observations occur (on a yearly basis)
ts_inflow_data
...
HP2_acf= acf(ts_inflow_data[,2] , lag = 60)
HP3_acf= acf(ts_inflow_data[,3] , lag = 60)

HP1_pacf= Pacf(ts_inflow_data[,1] , lag = 60)
HP2_pacf= pacf(ts_inflow_data[,2] , lag = 60)
HP3_pacf= pacf(ts_inflow_data[,3] , lag = 60)
