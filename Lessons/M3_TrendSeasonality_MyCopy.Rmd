...
#create empty lists for recursion
linear_trend <- list()
beta0 <- list()
beta1 <- list()
linear_trend_eq <- matrix()
detrend_inflow <- matrix()

#loop to recursively calculate detrended series
for (i in 2:length(inflow_data)) {
  #Fit a linear trend
  linear_trend[i-1] <- lm(inflow_data[,i]~t)
  
  #first coefficient is the intercept term or beta0
  beta0[i-1] <- as.numeric(linear_trend[[i-1]][1])
  #second coefficient is the slope or beta1
  beta1[i-1] <- as.numeric(linear_trend[[i-1]][2])
  
  #detrend
  linear_trend_eq[,i-1] <- beta0[i-1] + beta1[i-1]*t
  detrend_inflow[,i-1] <- inflow_data[,i] - linear_trend_eq[i-1]
}
...
#detrend
linear_trend_eq <- beta0 + beta1*t
detrend_inflow <- inflow_data[,2] - linear_trend_eq
