...
```{r}
ts_wind_data <- ts(wind_data$AWND.1,
                   frequency=12,
                   start = c(1984,1)) 
...
```{r}
clean_outliers_ts_wind <- forecast::na.interp(ts_wind_data)

autoplot(clean_wind_data, series="Wind Clean") +
  autolayer(clean_outliers_ts_wind, series = "Wind Clean with Outliers")+
  #autolayer(ts_wind_data, series="Wind Raw", lwd = 0.5) +
  ylab("Average Wind Speed") 
```
